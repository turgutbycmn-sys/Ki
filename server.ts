...
async function processVideoJob(
  ...
  try {
    ...
    const finalCommentary = logText || generateDefaultCommentaryForTitle(finalTitle, duration);
...
1. Identify all high-value football highlights (Goals, Saves, Skills, Key Passes, Counter Attacks, Defensives).
2. For each highlight, assign exact timestamp seconds within the range [0, ${duration}].
3. Compute Highlight Scores based on Event Importance (40%), Technical Difficulty (20%), Match Importance (15%), Tempo & Dynamics (10%), Emotional Impact (10%), and Visual Quality (5%), plus additive bonus modifiers (Solo run over 30m, Nutmeg, Backheel, One-touch, High press goal, Last-minute goal).
4. Provide Veo 3 tactical Animation coordinates for field replay board.
5. Provide strict real-life tracking player stats using "Player #<jersey>" naming convention.
...
`;
      ...
      // Ensure timestamps fit within duration
      if (parsedData.highlights && Array.isArray(parsedData.highlights)) {
        parsedData.highlights = parsedData.highlights.map((hl: any, idx: number) => {
          let secs = hl.startTimestamp || hl.seconds || Math.floor((idx + 1) * (duration / (parsedData.highlights.length + 1)));
          if (secs > duration) secs = Math.floor(duration * 0.8);
      ...
      console.warn(`[Job ${jobId}] Gemini API call failed or is unconfigured. Using intelligent local analyzer. Error:`, geminiError.message);
      matchData = localHeuristicAnalyzer(finalCommentary, finalTitle, competition, date, duration);
      matchData.id = "job-match-" + jobId;
      matchData.videoUrl = processingJobs[jobId]?.videoUrl;
      matchData.extractedFrames = frames;
    }
...
function generateDefaultCommentaryForTitle(title: string, durationSeconds: number): string {
  const parts = title.split(/\s+vs\s+/i);
  const teamA = parts[0] || "Home Team";
  const teamB = parts[1] || "Away Team";
  
  const t1 = formatTimeStr(durationSeconds * 0.15);
  const t2 = formatTimeStr(durationSeconds * 0.45);
  const t3 = formatTimeStr(durationSeconds * 0.75);
  const t4 = formatTimeStr(durationSeconds * 0.90);
